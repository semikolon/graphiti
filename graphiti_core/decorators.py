...
def handle_multiple_group_ids(func: F) -> F:
    ...
    FalkorDB uses separate graphs per group_id (unlike Neo4j which uses a single
    graph with group_id as a node property). When searching across multiple
    group_ids, this decorator runs the function for each group_id separately
    (with a driver cloned to point at that group_id's graph) and merges results.

    For single group_id or non-FalkorDB drivers, the function runs normally.
    """
    ...
    async def wrapper(self, *args, **kwargs):
        ...
        )  # Adjust for zero-based index (excludes self)
        ...
        # Normal execution (single group_id or non-FalkorDB)
