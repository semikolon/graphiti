...
- **`build_communities` MCP tool (SHIPPED + FalkorDB-verified 2026-06-10)** — clusters entities (label-propagation) + returns community summaries (the "themes across everything / overview" tier). Smoke-tested on FalkorDB (5 communities with summaries materialized). Cost scales with community count (one LLM summary each) → an occasional operation, not per-query.
