...
- **`reranker` param (SHIPPED + FalkorDB-verified 2026-06-10)** — all 6 search tools take `reranker`: `rrf` (default) / `mmr` (diversity; dedups near-identical results) / `cross_encoder` (precision) / `node_distance` (requires `center_node_uuid`). Empty preserves prior behaviour (node-distance when centered, else RRF). Helpers `_resolve_node/edge_search_config` + 12 unit tests (`test_reranker_selection.py`). MMR diversity rides the existing Qwen3 embeddings (Darwin :8080); cross-encoder rides the auto-init `OpenAIRerankerClient` — no new infra. **MMR gotcha (fixed in the helper):** the recipe's default `reranker_min_score=0` empties cohesive result sets (the MMR score floors at -1 and goes negative once results are mutually similar); the helper lowers it to `-2.0` so MMR reorders without dropping (live-verified 0→5 nodes).
- **`build_communities` MCP tool (SHIPPED + FalkorDB-verified 2026-06-10)** — clusters entities (label-propagation) + returns community summaries (the "themes across everything / overview" tier). Smoke-tested on FalkorDB. Cost scales with community count (one LLM summary each) → an occasional operation, not per-query. **Caveat (empirically + literature-confirmed 2026-06-10):** label-propagation is **non-deterministic** (random node-order + tie-breaking → different partitions across runs; a ~10-node test gave 5 then 6) AND **over-fragments small graphs** — degree-1/leaf nodes strand as singleton communities (a sparse 11-entity/9-edge test split the inkasso cluster into one triple + three identical-bureau singletons). Value grows with graph size (leaf-noise shrinks, real dense clusters emerge, instability averages out). So: treat as periodic + re-runnable, NOT a canonical one-shot; expect to merge singletons on small/medium graphs. Detail + sources: `~/Projects/din-mamma/docs/research/graphrag_landscape_second_pass_2026_06_09.md` § Community-detection caveat.

**Sequencing (✅ SHIPPED 2026-06-10):** Built the `reranker` param + `build_communities` tool on the fork's 0.20.1 — the MMR + cross-encoder recipes, reranker clients, and `build_communities` all already shipped in `graphiti_core`; only MCP *exposure* was missing (cross-encoder was even already live via the auto-init `OpenAIRerankerClient`). Do NOT gate on reintegration: upstream's MCP also wires only RRF + NODE_DISTANCE (reintegration wouldn't hand us MMR/cross-encoder for free), and a naive rebase would risk the fork's still-unmerged reliability patches (#1176 worker-GC, #1164 attr-collision — both OPEN upstream). Reintegration (0.20.1 → 0.29.x; 79 ahead / 237+ behind) is real but ORTHOGONAL debt → a separate, deliberately-scoped **rebuild-on-upstream** project (not a rebase); cherry-pick 0.29.1 attribute-hallucination guards + 0.28.2 Cypher-hardening opportunistically. HippoRAG/PPR is net-new (absent from both fork + upstream) → separate spike, lowest priority. Full analysis: `docs/upstream_divergence_and_reintegration_2026_06_10.md`. Reliability-patch provenance: `~/dotfiles/docs/graphiti_upstream_review_2026_03_15.md`. DIM consumer-side verdict: `~/Projects/din-mamma/docs/research/graphrag_landscape_second_pass_2026_06_09.md`.
